...
export default {
	...
	async created() {
		try {
			...
			toastRegistry.error(`[${this.name}] ${this.t("views.OptionsView.ChatsView.toast.chatBatchSizeError")}`)
	...
	methods: {
		...
		saveChatBatchSize: publicRegistry.debounce(async function (value) {
			...
			if (isNaN(BATCH_SIZE) || BATCH_SIZE <= 0) {
				...
				toastRegistry.error(`[${this.name}] ${this.t("views.OptionsView.ChatsView.toast.invalidChatBatchSize")}`)
				return
			}
			try {
				await this.$DB.configs.put({item: "chatBatchSize", value: value})
				toastRegistry.success(`[${this.name}] ${this.t("views.OptionsView.ChatsView.toast.chatBatchSizeSuccess")}`)
			} catch (error) {
